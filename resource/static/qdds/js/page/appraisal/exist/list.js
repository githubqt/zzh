...
var fields =  [ [ {
    ...
    formatter:function(value, row, index){  
    	var str = '<input type="button" onclick="location.href=\'/?m=Appraisal&c=Appraisalexist&a=detail&id='+row.id+'\'" class="easyui-linkbutton" data-options="selected:true" value="查看" >';
        ...
        if (row.appraisal_status == '30') {
        	str += '<input type="button" onclick="location.href=\'/?m=Appraisal&c=Appraisalexist&a=edit&id='+row.id+'\'" class="easyui-linkbutton" data-options="selected:true" value="补全资料" >';
        }
    ...
    $.messager.confirm('温馨提示', '您确定取消吗?', function (res) {
        if (res == true) {
            $.ajax({
                type: "POST",
                async: true,  // 设置同步方式
                url: "/?m=Appraisal&c=Appraisalexist&a=detail&format=cancel&id=" + id,
                dateType: "json",
                success: function (data) {
                    if (data.code == '200') {
                        location.href = "/index.php?m=Appraisal&c=Appraisalexist&a=list"
                    } else {
    ...
    $.messager.confirm('温馨提示', '您确定要收货吗?', function (res) {
        if (res == true) {
            $.ajax({
                type: "POST",
                async: true,  // 设置同步方式
                url: "/?m=Appraisal&c=Appraisalexist&a=detail&format=receiving&id=" + id,
                dateType: "json",
                success: function (data) {
                    if (data.code == '200') {
                        location.href = "/index.php?m=Appraisal&c=Appraisalexist&a=list"
...
// 		autoRowHeight: true,
// 		striped: true,
// 	    url: '/index.php?m=Appraisal&c=Appraisalexist&a=list&format=list',
// 		remoteSort: true,
// 		singleSelect:true,
// 		idField:'id',
// 		loadMsg:'数据加载中......',
