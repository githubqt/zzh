...
class PermissionModel extends \Common\CommonBase {
	...
	public static function getList($attribute = array(), $page = 0, $rows = 10) {
		...
		$sql .= " AND a.parent=0 GROUP BY a.id ORDER BY a.type,a.order_num";
	 ...
	 * @return array
	 */
	public static function getAllList($type = '') {
		$where = [ 
				'is_del' => '2' 
		];
		...
		$pdo = YDLib::getPDO ( 'db_r' );
		$ret = $pdo->clear ()->select ( '*' )->from ( self::$_tableName )->where ( $where )->getAll ();
