...
final class PDOQuery extends PDO {
	...
	public function __construct($params, $options) {
		...
		if (\Yaf_Application::app ()->getConfig ()->PDOParam)
			$opt [parent::MYSQL_ATTR_USE_BUFFERED_QUERY] = true;
	 ...
	 *        	表名
	 * @param array $where
	 *        	WHERE 条件数组，仅支持 AND 连接
	 * @param string $fields
	 *        	要查询的字段，半角逗号分隔，如：field1, field2
	 * @param null $order
	 *        	排序方法，如：someField DESC
	 * @param null $limit
	 *        	限制条数，可以是单个数字或者 array($offset, $num) 格式的数组
	 * @param null $group        	
	 * @param int $style        	
	 *
	 * @return mixed
	...
	public function from($tableName) {
		$this->_chained ['table'] = str_replace ( array (
				'`',
				'.' 
		), array (
				'',
	 ...
	 * 
	 * @param mixed $offset
	 *        	LIMIT 第一个参数，也可以写成 array($offset, $num) 以省略第二个参数
	 * @param null $num
	 *        	LIMIT 第二个参数
	 * @return $this
	 */
	public function limit($offset, $num = null) {
		$limit = is_null ( $num ) ? $offset : array (
				$offset,
				$num 
		);
		if (is_numeric ( $limit ))
	 ...
	 *        	记录格式，PDO::FETCH_*
	 * @param null $arg
	 *        	与 $style 对应的参数，目前只有 PDO::FETCH_COLUMN 需要
	 * @return array|bool
	 */
