...
//        ImagePng($QR);
    ...
    public function  qrcode($content,$detail)
    {
		...
		if (!file_exists($filePath)) {
			...
			//提前准备好的二维码
			$codeName = APPLICATION_PATH.'/application/resource/product/qrcode.png';
// 			$codeName = RESOURCE_STATIC."qdds/img/share/coupan/qrcode.png";
	        ...
	        if (!empty($detail['logo_url'])) {
				$goodImg = $detail['logo_url'];
// 				$goodImg = APPLICATION_PATH.'/application/resource/coupon.jpg';
			} else {
	        ...
	        for ($i=0; $i < 5; $i++) { 
			 ...
			 $sale_price = $detail['sale_price'] != '0' ? $detail['sale_price'] : $detail['channel_price'];
			
	        $topY += 120;
	        imagettftext($im, 36,0, $fontX, $topY, $font_color_2 ,$font_file, '本店销售：' . $sale_price);
	        $topY += 120;
			...
			imagettftext($im, $fontSize,0, ceil(($width - $fontBox[2]) / 2), 1250, $font_color_4 ,$font_file, $detail['company']);
			 ...
			 Header("Content-type: image/png");
			$code = ImagePng($im,RESOURCE_STATIC."common/images/promote/promote.png");
			
			if($code){
				return HOST_STATIC."common/images/promote/promote.png";
			}
			//return RESOURCE_STATIC."common/images/promote/promote.png";
...
/**
  ...
  function gbsubstr2($str, $start, $length) {
    ...
    public function scerweima($url=''){
    	$codeName = RESOURCE_STATIC.'common/images/qrcode/qrcode.png';
    	$value = $url;         //二维码内容
    	$errorCorrectionLevel = 'L';  //容错级别
    	$matrixPointSize = 5;      //生成图片大小
    	QRcode::png($value,$codeName, $errorCorrectionLevel, $matrixPointSize, 2);
    	//输出图片
    	return HOST_STATIC."common/images/qrcode/qrcode.png";
    }
