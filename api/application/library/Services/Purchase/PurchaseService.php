...
class PurchaseService extends BaseService
{
    ...
    const ORDER_CANCEL_SUPPORT = '取消';  // 供应取消
    /**
     * 退货订单状态
     *
     */
     ...
     * @return array|bool
     */
    public function purchaseReturn($child_order_no)
    {
        // 子单查询购买中的商品
    ...
    public function purchaseReceipt($purchase_id, $child_order_no)
    {
        ...
        if ($this->purchase['status'] != self::PURCHASE_ORDER_STATUS_50) {
            $this->error = '当前状态是:' . PurchaseModel::showOrderStatusText($this->purchase['status']) . ",不允许该操作";
            return false;
        }
    ...
    protected function handleReceipt($child_order_no)
    {
        ...
        try {
            ...
            switch ($this->purchase['purchase_type']) {
                case self::PURCHASE_TYPE_SALE:
                    // 库存处理
                    // TODO
                    break;
                case self::PURCHASE_TYPE_NEED:
                    // 库存处理
                    // TODO
                    break;
                case self::PURCHASE_TYPE_NORMAL:
                default:
                    // 库存处理
                    ...
                    foreach ($products as $item) {
                        ...
                        $PurchaseStockService->setType(PurchaseStockService::LOG_TYPE_12);
     ...
     * @return array|bool
     */
    public function queryExpress($child_order_no)
    {
        $order = PurchaseReturnModel::getExpressByChildOrderNo($child_order_no);
