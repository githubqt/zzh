...
class WxPayApi {
	...
	public static function refund($inputObj, $timeOut = 6) {
        ...
        $inputObj->SetAppid ( W_WEIXIN_APPID ); // 支付商户
        $inputObj->SetMch_id ( WEIXIN_MCHID ); // 支付商户ID号
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30) {
		YDLib::testlog ( $xml );
		YDLib::testlog ( $url );
		YDLib::testlog ( $useCert );
		YDLib::testlog ( $second );
		...
		YDLib::testlog ( $data );
		YDLib::testlog ( $ch );
		YDLib::testlog ( WxPayConfig::SSLCERT_PATH );
		YDLib::testlog ( WxPayConfig::SSLKEY_PATH );
		// 返回结果
		if ($data) {
			curl_close ( $ch );
			return $data;
