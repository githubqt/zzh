...
class Wechat {
    ...
    const OPEN_OAUTH_REFRESH_URL = '/sns/oauth2/component/refresh_token?';
...
,
	...
	public function __construct($options) {
        ...
        $this->api_authorizer_token = isset ( $options ['api_authorizer_token'] ) ? $options ['api_authorizer_token'] : '';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename, $value, $expired) {
		...
		$key = __CLASS__ . $cachename;
		return $mem->set ( $key, $value, $expired );
	}
    ...
    public function getOpenJsTicket($appid = '', $jsapi_ticket = '') {
        ...
        $result = $this->http_get ( self::API_URL_PREFIX . self::GET_TICKET_URL . 'access_token=' . $this->api_authorizer_token . '&type=jsapi' );
        if ($result) {
            $json = json_decode ( $result, true );
            if (! $json || ! empty ( $json ['errcode'] )) {
                $this->errCode = $json ['errcode'];
    ...
    public function getOpenJsSign($url, $timestamp = 0, $noncestr = '', $appid = '') {
        if (! $this->jsapi_ticket && ! $this->getOpenJsTicket ( $appid ) || ! $url)
            return false;
        if (! $timestamp)
            $timestamp = time ();
        if (! $noncestr)
	 ...
	 *        	卡券的类型，不可为空，官方jssdk文档说这个值可空，但签名验证工具又必填这个值，官方文档到处是坑，
	 ...
	 * @param string $appid
	 *        	用于多个appid时使用,可空
	 * @param string $api_ticket
	 *        	手动指定api_ticket，非必要情况不建议用
	 */
	 ...
	 *        	自定义追踪id,临时二维码只能用数值型
	 * @param int $type
	 *        	0:临时二维码；1:数值型永久二维码(此时expire参数无效)；2:字符串型永久二维码(此时expire参数无效)
	 * @param int $expire
	 *        	临时二维码有效期，最大为604800秒
    ...
    public function getOpenOauthAccessToken() {
        ...
        $result = $this->http_get ( self::API_BASE_URL_PREFIX . self::OPEN_OAUTH_TOKEN_URL . 'appid=' . $this->appid . '&component_appid=' . $this->component_appid . '&component_access_token=' . $this->component_access_token . '&code=' . $code . '&grant_type=authorization_code' );
        if ($result) {
	 ...
	 * @param string $order_id
	 *        	订单 ID
	 * @param int $need_delivery
	 *        	商品是否需要物流(0-不需要，1-需要)
	 * @param string $delivery_company
	 *        	物流公司 ID
	 * @param string $delivery_track_no
	 *        	运单 ID
	 * @param int $is_others
	 *        	是否为 6.4.5 表之外的其它物流公司(0-否，1-是)
	 * @return bool
	 */
