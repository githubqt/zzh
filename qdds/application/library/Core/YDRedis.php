...
final class YDRedis extends Redis
{
    ...
    public function setOptionYD($param) {
        if (!YDLib::checkData($param)) return false;
    ...
    public function hSetYD($hkey, $memKey, $memVal) {
        if (!YDLib::checkData([$hkey, $memKey, $memVal])) return false;
    ...
    public function hGetYD($hkey, $memKey){
        if (!YDLib::checkData([$hkey, $memKey])) return false;
    ...
    public function hLenYD($hkey) {
        if (!YDLib::checkData($hkey)) return false;
    ...
    public function hIncrByYD($hkey, $memKey, $val) {
        if (!YDLib::checkData([$hkey, $memKey, $val])) return false;
     ...
     * 
     * @param string $hkey
     * @param array $memKeys array(k=>v)
     * 
     * @return bool
     */
    public function hMsetYD($hkey, $memKeys) {
        if (!YDLib::checkData([$hkey, $memKeys])) return false;
     ...
     *
     * @param string $hkey
     * @param array $memKeys array(k1,k2)
     *
     * @return array|bool
