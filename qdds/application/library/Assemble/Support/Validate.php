...
class Validate
{
    ...
    // 是否验证supplier_id
    public $check_supplier_id = false;
    ...
    protected function unique($value, $rule, $data, $field)
    {
        ...
        if (false !== strpos($rule[0], '\\')) {
            ...
            try {
                $class = $this->getModelName($rule[0]);
                if ($class && class_exists($class)) {
                    $db = new $class();
                } else {
                    throw new \Exception("{$class} 模型不存在");
                }
            } catch (\Exception $e) {
        ...
        $pk = isset($rule[3]) ? $rule[3] : $db->getKeyName();
        if (is_string($pk)) {
            if (isset($rule[2])) {
                $map[$pk] = ['!=', $rule[2]];
        ...
        foreach ($map as $field => $value) {
            if ($i == 1) {
                ...
                $where .= " AND {$field} {$value[0]} '{$value[1]}' ";
            }
            $i++;
        }

        if ($this->check_supplier_id) {
            $auth = AdminModel::getCurrentLoginInfo();
            $where .= " AND supplier_id = '{$auth['supplier_id']}' ";
        }

        $sql = " SELECT {$pk} FROM " . $class::getFullTable() . " WHERE is_del= 2  AND {$where}";
        $row = $class::newRead()->YDGetOne($sql);
    ...
    protected function readDir($dir)
    {
        ...
        foreach ($dir_list as $file) {
            if ($file != '..' && $file != '.') {
                if (is_dir($dir . '/' . $file)) {
                    $d = $this->readDir($dir . '/' . $file);
                } else {
                    $d = $dir . '/' . $file;
                }

                if (!in_array($d, $this->models)) {
                    $models[] = $d;
                }
    ...
    {
        $root = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'application/models';
        $dirs = $this->readDir($root);
        $models = [];
        foreach ($dirs as $dir) {
            if (is_array($dir)) {
                foreach ($dir as $item) {
                    $models[] = str_replace($root, '', $item);
                }
            } else {
                $models[] = str_replace($root, '', $dir);
            }
        }
        foreach ($models as $model) {
            $model = str_replace('/', '\\', $model);
            $this->models[] = str_replace('.php', 'Model', $model);
        }
        return $this->models;
    }
    ...
    protected function getModelName($table)
    {
        ...
        foreach ($names as $name){
            $dirName .= "$name";
            $fullClassName = "\\$dirName$className";
            if (in_array($fullClassName, $this->models)) {
                return $fullClassName;
            }
        }
        $fullClassName || $fullClassName = $className;
        throw new \InvalidArgumentException("$fullClassName Not Found");
    }

    public static function validation($table = '')
    {
        $validate = new self();
        $model = $validate->getModelName($table);
     ...
     * @param $boolean
     */
    public function setCheckSupplierId($boolean = false)
    {
        $this->check_supplier_id = $boolean;
    }
