...
class Wechat
{
     ...
     * 获取微信服务器发来的信息
     */
    public function getRev($msg_signature,$timeStamp,$nonce)
    {
        ...
        if (!empty($postStr)) {
            ...
            if ($errCode == 0) {
                $msg = (array)simplexml_load_string($msg, 'SimpleXMLElement', LIBXML_NOCDATA);
                return $msg;
            }
            return $errCode;
        }
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    /**
     * xml格式加密消息，仅请求为加密方式时再用
     */
    public function generatemsg($fromUsername, $toUsername, $msgType, $contentStr,$nonce)
    {
        //格式化加密信息
        $textTpl = "<xml><ToUserName><![CDATA[%s]]></ToUserName><FromUserName><![CDATA[%s]]></FromUserName><CreateTime>%s</CreateTime><MsgType><![CDATA[%s]]></MsgType><Content><![CDATA[%s]]></Content></xml>";
        $xml = sprintf($textTpl, $fromUsername, $toUsername, time(), $msgType, $contentStr);
        $pc = new WXBizMsgCrypt (WEIXIN_OPEN_TOKEN, WEIXIN_OPEN_KEY, WEIXIN_OPEN_APPID );
        $msg = '';
        $errCode = $pc->EncryptMsg($xml, time(), $nonce, $msg);
        if ($errCode == '0') {
            return $msg;
        }
        return false;
    ...
    public function createMenu($data,$access_token = ''){
        if (!$this->access_token && !$access_token) return false;
        if ($access_token) {
     ...
     * @return boolean|array
     */
    public function uploadImg($access_token,$data){
        if (!$access_token) return false;
        //原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_UPLOADIMG_URL.'access_token='.$access_token,$data,true);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function uploadForeverMedia($access_token,$data, $type,$is_video=false,$video_info=array()){
        if (!$access_token) return false;
    ...
    public function uploadForeverArticles($access_token,$data){
        if (!$access_token) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_FOREVER_NEWS_UPLOAD_URL.'access_token='.$access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function updateForeverArticles($access_token,$media_id,$data,$index=0){
        if (!$access_token) return false;
        if (!isset($data['media_id'])) $data['media_id'] = $media_id;
        if (!isset($data['index'])) $data['index'] = $index;
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_FOREVER_NEWS_UPDATE_URL.'access_token='.$access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     */
    public function getForeverMedia($access_token,$media_id,$is_video=false){
        if (!$access_token) return false;
        $data = array('media_id' => $media_id);
        //#TODO 暂不确定此接口是否需要让视频文件走http协议
        //如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
        //$url_prefix = $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_FOREVER_GET_URL.'access_token='.$access_token,self::json_encode($data));
        if ($result)
        {
            if (is_string($result)) {
                $json = json_decode($result,true);
     ...
     * @return boolean
     */
    public function delForeverMedia($access_token,$media_id){
        if (!$access_token) return false;
        $data = array('media_id' => $media_id);
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_FOREVER_DEL_URL.'access_token='.$access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function sendCustomMessage($data,$access_token){
        if (!$access_token) return false;
     ...
     * @return boolean|array
     */
    public function setTMIndustry($access_token,$id1,$id2=''){
        if ($id1) $data['industry_id1'] = $id1;
        if ($id2) $data['industry_id2'] = $id2;
        if (!$access_token) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_SET_INDUSTRY_URL.'access_token='.$access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getIndustry($access_token){
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_GET_INDUSTRY_URL.'access_token='.$access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     */
    public function addTemplateMessage($access_token,$tpl_id){
        $data = array ('template_id_short' =>$tpl_id);
        if (!$access_token) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_ADD_TPL_URL.'access_token='.$access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getAllPrivateTemplate($access_token){
        if (!$access_token) return false;
        $data = array ('access_token' =>$access_token);
        $result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_GET_ALL_TPL_URL.'access_token='.$access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
    public function getCustomServiceKFlist($access_token){
        if (!$access_token) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::CUSTOM_SERVICE_GET_KFLIST.'access_token='.$access_token);
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * }
     */
    public function addKFAccount($account,$nickname,$password,$access_token){
        $data=array(
            "kf_account" =>$account,
            "nickname" => $nickname,
        ...
        if (!$access_token) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::CS_KF_ACCOUNT_ADD_URL.'access_token='.$access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
