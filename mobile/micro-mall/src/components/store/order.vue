<template>
    <section class="order-container">
        ...
        <yd-preview :buttons="btns" class="m-b-_24">
            <yd-preview-header>
                <div class="order-product-detail-box" slot="left" @click="detailsGo(productDetail.id)">
                    <img :src="productDetail.logo_url" :onerror="errorImg">
                </div>
                <div slot="right" class="order-product-detail-title">
                    <div style="white-space: normal;">{{productDetail.name}}</div>
                    <div class="order-product-detail-price">
                        <div class="market-price">
                            <span>公价:{{productDetail.market_price}}</span>
                        </div>
                        <div class="order-product-header-price">
                            <span class="emphasis" v-if="productDetail.seckill.length!==0">
                                <em>￥</em>{{productDetail.seckill.seckill_price}}
                            </span>
                            <span class="emphasis" v-else><em>￥</em>{{productDetail.sale_price}}</span>
        ...
        <yd-cell-group>
            <yd-cell-item arrow @click.native="seckillShow=true">
                ...
                <span slot="right" class="order-delivery-site">{{coupan_name}}</span>
            </yd-cell-item>
        </yd-cell-group>
        <yd-cell-group>
            <yd-cell-item>
        ...
        <yd-flexbox class="order-submit-btn">
            <yd-flexbox-item class="cart-button-right">
                <yd-button size="large" class="no-radius" type="danger" @click.native="orderSubmit">确认订单</yd-button>
            </yd-flexbox-item>
        </yd-flexbox>
        <!-- 选择快递方式 -->
        ...
        <!-- 选择优惠券  -->
        <yd-popup v-model="seckillShow" position="bottom" height="auto" class="order-popup-box">
            <yd-cell-group>
            ...
            <router-link to="/couponsCenter" class="order-btn-box" style="width: 50%;float: left;background-color:#dab461;color:#fff">去领券</router-link>
        </yd-popup>
    </section>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            address_phone: '',
            btns: [],
            productDetail: { seckill: { restrictions_num: 0 } },
            amount: 1,
            coupanData: '',
            coupan_id: '',
            coupan_name: '',
    ...
    methods: {
        ...
        // 商品详情
        detailProduct() {
            let _data = Qs.stringify({ id: this.$route.query.product_id });
        ...
        coupanGet() {
            let _data = Qs.stringify({
                ...
                product: '[{ "product_id": "' + this.$route.query.product_id + '", "num": "' + this.amount + '"}]',
            });
        ...
        assignment() {
            ...
            _this.$http.all([_this.siteList(), _this.defaultSite(), _this.detailProduct(), _this.coupanGet()])
                .then(_this.$http.spread(function(s, d, c, f) {
                    ...
                    _this.coupanData = f.data.result; // 优惠券列表
                    if(_this.coupanData) {
                        _this.coupanData.forEach(item => {
        ...
        getMoney() {
            let _this = this,
                _data = Qs.stringify({
                    ...
                    seckill_id: _this.productDetail.seckill.seckill_id,
                    product: '[{ "product_id": "' + _this.$route.query.product_id + '", "num": "' + _this.amount + '"}]',
                    user_coupan_id: _this.coupan_id
                });

            _this.$dialog.loading.open('很快加载好了');
            _this.$http.post('/api/v1/Order/getMoney', _data).then(function(response) {
        ...
        orderSubmit() {
            ...
            _this.$dialog.loading.open('很快加载好了');
            _this.$http.post('/api/v1/Order/add', _data).then(function(response) {
                if (response.data.errno === '0') {
                    _this.$dialog.loading.close();
                    var payurl = '';
        ...
        detailsGo(id) { this.$router.push({ name: 'Details', query: { id: id } }) }
    }
}
